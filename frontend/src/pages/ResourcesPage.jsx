...
const ResourcesPage = () => {
  ...
  const { data: serversRaw, isLoading } = useQuery({
    queryKey: ['resources-servers'],
    queryFn: () => metricsService.getServers().then(r => r.data?.data || r.data || []),
    refetchInterval: 15000,
    retry: 2,
  })

  const servers = Array.isArray(serversRaw) ? serversRaw.map((s, i) => ({
    id: s.id || i,
    name: s.hostname || s.name || `server-${i}`,
    ip: s.ip || s.ipAddress || '-',
    cpu: s.cpu ?? 0,
    ram: s.ram ?? 0,
    disk: s.disk ?? 0,
  ...
  const filteredServers = activeFilter === 'Todos'
    ? servers
    : servers.filter(s => s.status === activeFilter.toLowerCase())
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-text-muted">
        <motion.div
          className="w-10 h-10 rounded-full border-2 border-accent-cyan/20 border-t-accent-cyan mb-4"
          animate={{ rotate: 360 }}
          transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}
        />
        <p className="text-xs">Cargando servidores...</p>
      </div>
    )
  }
          ...
          <GlassCard padding="p-12">
            <div className="flex flex-col items-center justify-center text-center">
              <div className="w-16 h-16 rounded-2xl bg-accent-cyan/5 border border-accent-cyan/10 flex items-center justify-center mb-5">
                <Icon name="server" size={28} className="text-accent-cyan/40" />
              </div>
              <h3 className="text-lg font-semibold text-text-primary mb-2">Sin Servidores Conectados</h3>
              <p className="text-xs text-text-secondary max-w-sm mb-1">
                Instala el agente de HawkScope en tu servidor para comenzar a monitorear tu infraestructura en tiempo real.
              </p>
            ...
            {filteredServers.map((server, idx) => (
              ...
              >
                {viewMode === 'grid' ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {server.region !== '-' && <span className="text-[8px] text-text-muted px-1.5 py-0.5 rounded bg-surface-3 border border-white/[0.04]">{server.region}</span>}
                          </div>
                        </div>
                      </div>
                  ...
                  <GlassCard padding="px-4 py-3" hover={false} className="hover:border-accent-cyan/10 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      {server.status !== 'offline' && (
                        <div className="hidden md:flex items-center gap-6 text-[10px]">
                          <div><span className="text-text-muted">CPU</span><span className={`ml-1.5 font-mono font-medium ${server.cpu > 80 ? 'text-status-critical' : 'text-text-primary'}`}>{server.cpu}%</span></div>
                          <div><span className="text-text-muted">RAM</span><span className="ml-1.5 font-mono font-medium text-text-primary">{server.ram}%</span></div>
                          <div><span className="text-text-muted">DISK</span><span className="ml-1.5 font-mono font-medium text-text-primary">{server.disk}%</span></div>
                        </div>
                      )}
                      <StatusBadge status={server.status} size="xs" />
                    </div>
